...
export function boardToFen(board: SquareData[], currentPlayer: "white" | "black") {
  let fen = "";
  let emptyPieces = 0;
...
function calculateHasMoved(
  ...
  if (pieceType === "king") {
    if (color === "white") {
      return row !== 1 || col !== 5 || (!castling.includes("K") && !castling.includes("Q"));
    } else {
      return row !== 8 || col !== 5 || (!castling.includes("k") && !castling.includes("q"));
    }
  }
...
export function fenToBoard(fen: string): SquareData[] {
  ...
  position.split("").forEach((e: string) => {
    ...
    const pieceKey = Object.keys(pieceTypeMap).find((key) => pieceTypeMap[key] === lowerChar) as
      | keyof typeof pieceTypeMap
      | undefined;
